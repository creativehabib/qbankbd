                    ...
                    <div x-data="{ expanded: false}">
                        <div class="text-sm text-zinc-600 dark:text-zinc-400 leading-relaxed mb-4 transition-all duration-300 ease-in-out overflow-hidden" :class="expanded ? 'max-h-[1000px]' : 'max-h-[46px] line-clamp-2'">
                            @if($exam->description)
                                {!! $exam->description !!}
                            @else
                                {{ $institution->name }} এর {{ $exam->title }} পদের নিয়োগ পরীক্ষাটি সরকারি চাকরিপ্রার্থীদের জন্য একটি গুরুত্বপূর্ণ পরীক্ষা। এই পরীক্ষায় সাধারণত {{ $exam->type }} পদ্ধতিতে প্রশ্ন এসে থাকে। নিচে সম্পূর্ণ সমাধান দেওয়া হলো।
                            @endif
                        </div>
