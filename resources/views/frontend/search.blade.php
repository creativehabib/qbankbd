...
@section('title', 'অনুসন্ধান ফলাফল')
@section('description', 'প্রশ্ন ব্যাংক এ প্রশ্ন অনুসন্ধান করুন।')
    ...
    <div class="max-w-4xl mx-auto space-y-6">
        ...
        @if(!empty($query))
            @if($results && $results->count() > 0)
                <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 p-6">
                    <p class="text-sm text-slate-500 dark:text-slate-400 mb-6">"{{ $query }}" এর জন্য {{ $results->total() }} টি ফলাফল পাওয়া গেছে।</p>
            ...
            @else
                <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 p-12 text-center">
                    ...
                    <h3 class="text-lg font-bold text-slate-800 dark:text-white mb-2">কোনো ফলাফল পাওয়া যায়নি</h3>
                    <p class="text-slate-500 dark:text-slate-400">"{{ $query }}" এর জন্য কোনো প্রশ্ন পাওয়া যায়নি। অন্য কোনো শব্দ দিয়ে চেষ্টা করুন।</p>
                </div>
            @endif
        @endif
    </div>
