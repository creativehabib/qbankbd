...
<div class="flex min-h-screen">
    <aside
        class="fixed inset-y-0 left-0 z-40 hidden w-72 flex-col border-e border-zinc-200 bg-zinc-50 transition-all duration-200 dark:border-[var(--app-dark-border)] dark:bg-[var(--app-dark-panel)] lg:flex"
        data-test="desktop-sidebar"
    >
        <div class="flex items-center justify-between border-b border-zinc-200 px-4 py-3 dark:border-[var(--app-dark-border)]">
        ...
        <nav class="flex-1 space-y-2 overflow-y-auto px-3 py-4" data-test="sidebar-nav">
            <a
                ...
                class="flex items-center gap-2 rounded-lg px-3 py-2 text-sm font-medium transition {{ request()->routeIs('dashboard') ? 'bg-zinc-900 text-white dark:bg-zinc-100 dark:text-zinc-900' : 'text-zinc-700 hover:bg-zinc-200 dark:text-zinc-100 dark:hover:bg-zinc-800' }}"
                wire:navigate
            >
                {{ __('Dashboard') }}
            </a>
            ...
            >
                <button
                    type="button"
                    class="flex w-full items-center justify-between rounded-lg px-3 py-2 text-left text-sm font-semibold text-zinc-700 hover:bg-zinc-200 dark:text-zinc-100 dark:hover:bg-zinc-800"
                    @click="open = ! open"
                >
                ...
                <div x-show="open" x-collapse class="mt-1 space-y-1 px-1 pb-1">
            ...
            <a href="{{ route('questions.set.create') }}" class="block rounded-lg px-3 py-2 text-sm {{ request()->routeIs('questions.set.create.*') ? 'bg-zinc-900 text-white dark:bg-zinc-100 dark:text-zinc-900' : 'text-zinc-700 hover:bg-zinc-200 dark:text-zinc-100 dark:hover:bg-zinc-800' }}" wire:navigate>{{ __('Question Create') }}</a>

            @if(auth()->user()->hasRole(['teacher', 'admin', 'super_admin']))
                <a href="{{ route('omr.generator') }}" class="block rounded-lg px-3 py-2 text-sm {{ request()->routeIs('omr.generator') ? 'bg-zinc-900 text-white dark:bg-zinc-100 dark:text-zinc-900' : 'text-zinc-700 hover:bg-zinc-200 dark:text-zinc-100 dark:hover:bg-zinc-800' }}" wire:navigate>{{ __('OMR Generator') }}</a>
            @endif

            @if(auth()->user()->isStudent())
                <a href="{{ route('students.practice.index') }}" class="block rounded-lg px-3 py-2 text-sm {{ request()->routeIs('students.practice.*') ? 'bg-zinc-900 text-white dark:bg-zinc-100 dark:text-zinc-900' : 'text-zinc-700 hover:bg-zinc-200 dark:text-zinc-100 dark:hover:bg-zinc-800' }}" wire:navigate>{{ __('Practice') }}</a>
            @endif

            @if(auth()->user()->hasPermission('users.manage_roles'))
                <div class="space-y-1 rounded-xl border border-zinc-200/80 p-2 dark:border-zinc-800">
            ...
            <a href="https://laravel.com/docs/starter-kits#livewire" target="_blank" class="block rounded-lg px-3 py-2 text-sm text-zinc-700 hover:bg-zinc-200 dark:text-zinc-100 dark:hover:bg-zinc-800">{{ __('Documentation') }}</a>
        </nav>

        <div class="border-t border-zinc-200 p-3 dark:border-[var(--app-dark-border)]">
    ...
    <div class="min-h-screen flex-1 lg:pl-72">
        ...
        <aside
            x-show="mobileSidebarOpen"
            x-transition
            class="fixed inset-y-0 left-0 z-50 w-72 overflow-y-auto border-e border-zinc-200 bg-zinc-50 p-3 dark:border-[var(--app-dark-border)] dark:bg-[var(--app-dark-panel)] lg:hidden"
        >
