        ...
        <div class="overflow-x-auto border-t border-zinc-200 dark:border-zinc-700">
            <flux:table>
                ...
                <flux:table.rows>
                        ...
                        <flux:table.row wire:key="question-{{ $quickFilter }}-{{ $question->id }}">
                            ...
                            <flux:table.cell class="min-w-80">
                                ...
                                <div class="mt-1 flex flex-wrap items-center gap-2 text-xs text-zinc-500 dark:text-zinc-400">
                                    <span>#{{ $question->id }}</span><span>•</span><span>{{ $question->user?->name ?? 'Unknown teacher' }}</span><span>•</span><span>{{ $question->marks }} mark(s)</span>
                                </div>
                            </flux:table.cell>
                            <flux:table.cell>
                    ...
                    @empty
                        <flux:table.row><flux:table.cell :colspan="$canDeleteQuestion ? 6 : 5" class="py-14 text-center text-zinc-500">No questions found for the selected filters.</flux:table.cell></flux:table.row>
                    @endforelse
                </flux:table.rows>
            </flux:table>
        </div>
    ...
    <flux:modal wire:model="showQuestionModal" class="w-full max-w-4xl">
        @if($selectedQuestion)
            <div class="space-y-5">
                ...
                <flux:card class="!p-4"><div class="prose max-w-none dark:prose-invert">{!! $selectedQuestion->title !!}</div>@if($selectedQuestion->description)<div class="mt-4 border-t pt-4 text-sm text-zinc-600 dark:text-zinc-300">{!! $selectedQuestion->description !!}</div>@endif</flux:card>
