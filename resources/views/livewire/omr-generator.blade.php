<div class="p-4 space-y-4 bangla">
    @php($themeClasses = $this->themeClassSet())
    ...
    <div class="flex gap-2 flex-wrap">
        @for ($i = 1; $i <= $questionCount; $i++)
            <div class="flex mt-[1px] border {{ $themeClasses['border'] }} w-[160px]">
                <div class="w-[38px] text-center border-r p-[1px] {{ $themeClasses['border'] }} text-xs">
                    {{ $this->toBanglaNumber($i) }}
                </div>
                <div class="text-center flex-1 grid grid-cols-4">
                    @foreach (['ক', 'খ', 'গ', 'ঘ'] as $option)
                        <div class="{{ $themeClasses['bg'] }} border-r last:border-r-0 {{ $themeClasses['border'] }} center p-[1px]">
