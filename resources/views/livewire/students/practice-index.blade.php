...
@push('scripts')
    <script>
        ...
        window.renderPracticeMath = function () {
            if (!window.MathJax || typeof window.MathJax.typesetPromise !== 'function') {
                return;
            }

            const elements = document.querySelectorAll('[data-math-content]');
        ...
        if (!document.getElementById('practice-mathjax-script')) {
            const script = document.createElement('script');
            script.id = 'practice-mathjax-script';
            script.src = 'https://cdn.jsdelivr.net/npm/mathjax@3/es5/tex-svg.js';
            script.async = true;
            script.onload = () => window.renderPracticeMath();
            document.head.appendChild(script);
        } else {
            window.renderPracticeMath();
        }
